...
纯收益率优化，适合追求高收益的激进型投资者

用法:
    python max_return_optimizer.py --start 20240101 --end 20241231 --timeframe d1

特性:
- 纯收益率排序（无夏普要求）
- 三层优化架构：收益筛选 → 相关性分组 → 收益权重
- 4种权重方法：收益加权、最大收益、等权、夏普加权（对比）
...
class MaxReturnOptimizer:
    ...
    def optimize(
        ...
        max_drawdown: float = -20.0,  # 比夏普优化器的-10%更宽松
        ...
        if weight_methods is None:
            weight_methods = ['return_weighted', 'max_return', 'equal_weight', 'sharpe_weighted']

        print("\n" + "=" * 80)
        print("最大收益率投资组合优化器（激进模式）")
        print("=" * 80)
        ...
        # === 步骤1: 质量筛选（纯收益排序） ===
        print("\n" + "=" * 80)
        print("步骤1: 策略收益率筛选（激进模式 - 无夏普要求）")
        print("=" * 80)

        high_return_strategies = self.quality_filter.filter_strategies_by_return(
            min_return=min_return,
            max_drawdown=max_drawdown,
            top_n=top_n_quality
        )

        self.quality_filter.print_return_quality_report(high_return_strategies)

        if len(high_return_strategies) < min_strategies_per_portfolio:
            print(f"\n错误: 高收益策略数量不足 ({len(high_return_strategies)} < {min_strategies_per_portfolio})")
            print("建议: 放宽筛选条件（降低 min_return）")
            return pd.DataFrame()

        print(f"通过收益率筛选: {len(high_return_strategies)} 个策略")
        ...
        filtered_returns = {
            ...
            if k in high_return_strategies
        }
        ...
        # 绘制热力图（仅高收益策略）
        heatmap_file = f'{self.results_dir}/correlation_heatmap_maxreturn_{self.timeframe}_{self.start_date}_{self.end_date}.png'
        ...
        # 按预期收益率排序（关键差异！）
        portfolios_df = portfolios_df.sort_values('expected_return', ascending=False)
        ...
        # === 步骤5: 保存结果 ===
        output_file = f'{self.results_dir}/max_return_portfolios_{self.timeframe}_{self.start_date}_{self.end_date}.csv'
        portfolios_df.to_csv(output_file, index=False)

        print(f"\n最大收益组合已保存: {output_file}")
    ...
    def _generate_low_correlation_combinations(
        ...
        strategies_sorted = sorted(
            strategies,
            key=lambda s: quality_scores.get(s, {}).get('total_return', 0),
            reverse=True
        )

        print(f"  策略排序（按收益率）: {strategies_sorted[:5]}...")
    ...
    def _print_top_portfolios(self, portfolios_df: pd.DataFrame, top_n: int = 10):
        """打印前N个推荐组合（按收益率排序）"""
        print("\n" + "=" * 100)
        print(f"前 {top_n} 个推荐组合（按预期收益率排序）")  # 修改标题
        print("=" * 100)
        print(f"{'ID':<4} {'策略数':<6} {'权重方法':<18} {'预期收益%':<12} {'预期夏普':<10} {'预期回撤%':<12}")  # 调整列顺序
        print("-" * 100)

        for idx, row in portfolios_df.head(top_n).iterrows():
            print(f"{row['portfolio_id']:<4} {row['num_strategies']:<6} "
                  f"{row['weight_method']:<18} {row['expected_return']*100:<12.2f} "  # 收益率放前面
                  f"{row['expected_sharpe']:<10.2f} {row['expected_max_dd']*100:<12.2f}")
...
def main():
    parser = argparse.ArgumentParser(description='最大收益率投资组合优化器（激进型）')
    ...
    parser.add_argument('--max-drawdown', type=float, default=-20.0, help='最大回撤%% (默认: -20.0)')
    parser.add_argument('--top-n-quality', type=int, default=20, help='选择收益率前N个 (默认: 20)')
    ...
    parser.add_argument('--weight-methods', nargs='+',
                       default=['return_weighted', 'max_return', 'equal_weight', 'sharpe_weighted'],
                       help='权重方法列表 (默认: return_weighted max_return equal_weight sharpe_weighted)')
    ...
    if not portfolios_df.empty:
        ...
        print(f"    --portfolio-file {args.results_dir}/max_return_portfolios_{args.timeframe}_{args.start}_{args.end}.csv")
        print()
    else:
        print("\n优化失败，请检查参数设置")
